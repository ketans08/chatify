...
export const sendMessage = async (req, res) => {
    try {
        ...
        if(!text && !image){
            return res.status(400).json({message:"Test or Image is required."});
        }
...
export const getChatPartners = async (req, res) => {
    try {
        ...
        //find all the messages where all the user is either send or reciever
        ...
        console.log("Error in get Chat Partner controller: ", error.message);
        res.status(500).json({ error: "Internal server error" });
    }
}